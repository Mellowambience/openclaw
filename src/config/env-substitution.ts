...
export type ResolveConfigEnvVarsOptions = {
  /**
   * When `true`, unresolvable `\${VAR}` references are preserved as-is instead
   * of throwing `MissingEnvVarError`.
   *
   * Use this for CLI commands that load config for read-only inspection (e.g.
   * `openclaw doctor`, `openclaw gateway status`).  Those commands don't need
   * the value of every env var — they only need a structurally valid config
   * object.  Gateway-only vars (injected via a systemd `EnvironmentFile=`)
   * won't be present in the CLI process environment, so strict substitution
   * would fail even though the CLI command itself doesn't use those values.
   *
   * @default false
   */
  lenient?: boolean;
  ...
  for (let i = 0; i < value.length; i += 1) {
    ...
    if (token?.kind === "substitution") {
      ...
      if (envValue === undefined || envValue === "") {
        if (options.lenient) {
          // Preserve the original \${VAR} token — the CLI doesn't need this value.
          chunks.push(`\${${token.name}}`);
          i = token.end;
          continue;
        }
 ...
 * @param options - Optional behaviour flags (e.g. `lenient` to preserve unresolvable refs)
